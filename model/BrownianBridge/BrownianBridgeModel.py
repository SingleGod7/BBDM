...
class BrownianBridgeModel(nn.Module):
    ...
    def p_sample_loop(self, y, context=None, clip_denoised=True, sample_mid_step=False):
        ...
        if sample_mid_step:
            ...
            for idx, ose in enumerate(imgs):
                # 假设img形状为 (B, C, H, W)，这里只保存第一个batch
                img_to_save = ose[0].detach().cpu()
                img_to_save = img_to_save.mul_(0.5).add_(0.5).clamp_(0, 1.)
                vutils.save_image(img_to_save, os.path.join(save_dir, f"step_{idx:03d}.png"))
