...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def sample_to_eval(self, net, test_loader, sample_path):
        ...
        gt_path = make_dir(os.path.join(sample_path, 'ground_truth_np'))
        result_path = make_dir(os.path.join(sample_path, str(self.config.model.BB.params.sample_step)))
        vis_path = make_dir(os.path.join(sample_path, 'vis'))
        np_path = make_dir(os.path.join(sample_path, 'result_np'))
            ...
            for j in range(sample_num):
                ...
                for i in range(batch_size):
                    ...
                    if j == 0:
                        ...
                        save_single_image(torch.concatenate([condition, result, gt], axis=2), vis_path, f'{x_name[i]}.png', to_normal=to_normal)
                        gt = gt.mul_(0.5).add_(0.5).clamp_(0, 1.)
                        np.save(os.path.join(gt_path, f'{x_name[i]}'), gt.cpu().numpy())
                    if sample_num > 1:
                        ...
                        result = result.mul_(0.5).add_(0.5).clamp_(0, 1.)
                        np.save(os.path.join(np_path, f'{x_name[i]}'), result.cpu().numpy())
